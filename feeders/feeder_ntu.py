...
class Feeder(Dataset):
    ...
    def process_and_save(self, output_path = 'NTU120_ppc_test.npz'):
        print("output_path: ", output_path)
        all_data_numpy = np.array(self.data)
        all_labels = np.array(self.label)
        all_sample_name = np.array(self.sample_name)
        ...
        np.savez(output_path, data=processed_data, labels=all_labels,num_frames=valid_frame_nums, name=all_sample_name)
...
class Feeder_attack(Dataset):
    ...
    def load_data(self):
        ...
        npz_data = np.load(self.data_path)
        if self.split == 'train':
            # self.data = np.load('./data/ntu_attack/iter_100_thres0.30_train.npy',allow_pickle=True)
            # self.data = np.load('./adv_data/ntu120/benign_train_NTU120_purified_multi.npz',allow_pickle=True)
            self.data = np.load('./adv_data/ntu60/benign_train_purified_total.npz',allow_pickle=True)
            self.data = self.data['data']
            self.label = np.where(npz_data['y_train'] > 0)[1]
            self.sample_name = ['train_' + str(i) for i in range(len(self.data))]
            # with open('./adv_data/ntu60/train_label.pkl', 'rb') as f:
    ...
    def process_and_save(self, output_path = 'processed_test_data.npz'):
        all_data_numpy = np.array(self.data)
        all_labels = np.array(self.label)
...
class FeederEG(Dataset):
    ...
    def load_data(self):
        ...
        #v_data = np.load(self.data_path)
        print(self.data_path)
        #self.data = v_data['data']
        #self.label = v_data['labels']
        #self.sample_name = v_data['sample_name']
        npz_data = np.load(self.data_path)
        if self.split == 'train':
            self.data = npz_data['x_test']
            self.label = np.where(npz_data['y_test'] > 0)[1]
            ...
            self.data = self.data.reshape((N, T, 2, 25, 3)).transpose(0, 4, 1, 3, 2)
            print("테스트데이터 저장")
            self.process_and_save('NTU60_ppc_test.npz')
            exit()
    ...
    def process_and_save(self, output_path = 'NTU60_ppc_train.npz'):
        print("output_path: ", output_path)
        all_data_numpy = np.array(self.data)
        all_labels = np.array(self.label)
        all_sample_name = np.array(self.sample_name)
